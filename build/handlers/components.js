...
export default async function handleComponents(countrData, components, premium = false) {
    ...
    for (const component of components.filter(({ name }) => (premium ? /^Premium Shard (\d+)$/u : /^Shard (\d+)$/u).exec(name))) {
        ...
        if (update)
            updates.push({ id: component.id, ...update });
    }
...
function formUpdate(shard, existingComponent = {}) {
    const update = {};
    if (shard?.status === Status.Ready && !shard.loading) {
        const description = `${shard.guilds} guilds, ${shard.users} users, ${shard.ping}ms ping`;
        if (existingComponent.description !== description)
            update.description = description;
        if (existingComponent.status !== "OPERATIONAL")
            update.status = "OPERATIONAL";
    }
    else {
        if (existingComponent.description !== "")
            update.description = "";
        const status = getStatus(shard?.status ?? Status.Disconnected);
        if (existingComponent.status !== status)
...
function getStatus(status) {
    ...
    if (status === Status.Idle)
        return "PARTIALOUTAGE";
    if (status === Status.Nearly)
        return "DEGRADEDPERFORMANCE";
    if (status === Status.Disconnected)
        return "MAJOROUTAGE";
    if (status === Status.WaitingForGuilds)
        return "PARTIALOUTAGE";
